...
def anova_analysis(df):
    """
    对DataFrame df进行单因素ANOVA分析，比较不同evt组的values差异。
    假设df包含以下列：
    - 'values': 数值数据
    - 'evt': 分组变量（类别型）
    """
...
def anova_cluster_permutation_test(df):
    """
    对DataFrame df进行单因素ANOVA分析，并执行基于聚类的置换检验。
    假设df包含以下列：
    - 'time': 时间点
    - 'values': 数值数据
    - 'mode': 模式（类别型）
    ...
    def cluster_permutation_test(p_series, alpha=0.05, n_permutations=500):
        """
        对时间序列p值进行聚类置换检验
